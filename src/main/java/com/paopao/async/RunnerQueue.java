package com.paopao.async;


import com.paopao.async.asyncRunner.BaseRunner;
...
public class RunnerQueue {
    ...
    @Value("${paopao.runner.queue.max.size}")
    ...
    public synchronized boolean addNewRunner(BaseRunner baseRunner){
        if(baseRunner == null){
            SysLogger.info("Runner为空,无法添加任务");
            return false;
        }

        //队列已满,要求申请方稍后重试
        if(size >= MAX_SIZE){
            SysLogger.info("消息队列已经满员，请稍后重试");
            return false;
        }
